...
const ChatMessages = ({ sessionId, setSessionId }: { sessionId: string; setSessionId: (sessionId: string) => void }) => {
  ...
  const { data: chatHistory, isLoading: isChatHistoryLoading, refetch: chatHistoryRefetch, error: chatHistoryError } = useChatHistory({ sessionId });
  const {
    mutate: sendMessage,
    isFirstChunk,
  ...
  useEffect(() => {
    if (isHttpError(chatHistoryError) && chatHistoryError.status === 401) router.push("/signin");
  }, [chatHistoryError, router]);
